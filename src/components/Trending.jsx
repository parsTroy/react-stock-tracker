...
const Trending = () => {
    ...
    return (
        <div className='rounded-div my-12 py-8 text-primary'>
            <h1 className='text-2xl font-bold py-4'>Trending Stocks</h1>
                ...
                {trending.map((stock, idx) => {
                    <div key={idx} className='rounded-div flex justify-between p-4 hover:scale-105 ease-in-out duration-300'>
                        <div className='flex w-full items-center justify-between'>
                            <div className='flex'>
                                <img className='mr-4 rounded-full' src={stock.item.small} alt="/" />
                                <div>
                                    <p className='font-bold'>{stock.item.name}</p>
                                    <p>{stock.item.symbol}</p>
                                </div>
                            </div>
                            <div className='flex items-center'>
                                <img className='w-4 mr-2' src='https://assets.coingecko.com/coins/images/1/large/bitcoin.png?1547033579' alt='/' />
                                <p>{stock.item.price_btc.toFixed(7)}</p>
                            </div>
                        </div>
                    </div>
                })}
